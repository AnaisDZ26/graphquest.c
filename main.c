...
Escenario* cargarEscenarios(const char* nombreArchivo) {
    ...
    for (int i = 0; i < cantidad; i++) {
        ...
        for (int j = 0; j < cantidad; j++) {
            if (escenarios[i].id_up == escenarios[j].id)
                escenarios[i].arriba = &escenarios[j];
            if (escenarios[i].id_down == escenarios[j].id)
                escenarios[i].abajo = &escenarios[j];
            if (escenarios[i].id_left == escenarios[j].id)
                escenarios[i].izquierda = &escenarios[j];
            if (escenarios[i].id_right == escenarios[j].id)
                escenarios[i].derecha = &escenarios[j];
        }
    }
...
void mostrarTurno(Partida* partida) {
    for(int i = 0; i < partida->n_jugadores; i++) {
        ...
        if(jugador->escenario->cantidadObjetos > 0) {
            ...
            puts("No hay objetos disponibles en este escenario.");
        }
