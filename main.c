...
Escenario* cargarEscenarios(const char* nombreArchivo) {
    ...
    while (fgets(linea, sizeof(linea), archivo)) {
        ...
        *arribaEnd = '\0';
        escenario->arriba = (atoi(token) == -1) ? NULL : (Escenario*)atoi(token);
        token = arribaEnd + 1;
        ...
        *abajoEnd = '\0';
        escenario->abajo = (atoi(token) == -1) ? NULL : (Escenario*)atoi(token);
        token = abajoEnd + 1;
        ...
        *izqEnd = '\0';
        escenario->izquierda = (atoi(token) == -1) ? NULL : (Escenario*)atoi(token);
        token = izqEnd + 1;
        ...
        *derEnd = '\0';
        escenario->derecha = (atoi(token) == -1) ? NULL : (Escenario*)atoi(token);
        token = derEnd + 1;
    ...
    for (int i = 0; i < cantidad; i++) {
        ...
        // Buscar y establecer conexiones
        for (int j = 0; j < cantidad; j++) {
            if ((int)escenario->arriba == escenarios[j].id) escenario->arriba = &escenarios[j];
            if ((int)escenario->abajo == escenarios[j].id) escenario->abajo = &escenarios[j];
            if ((int)escenario->izquierda == escenarios[j].id) escenario->izquierda = &escenarios[j];
            if ((int)escenario->derecha == escenarios[j].id) escenario->derecha = &escenarios[j];
        }
    }
...
void mostrarTurno(Partida* partida) {
    for(int i = 0; i < partida->n_jugadores; i++) {
        ...
        if(jugador->escenario->cantidadObjetos > 0) {
            printf("\nTomar objeto\n");
            for(int i = 0; i < jugador->escenario->cantidadObjetos; i++) {
        ...
        do {
            ...
            switch(opcion) {
                ...
                case '1':
                    if(jugador->escenario->cantidadObjetos > 0) {
                        jugador->inventario[jugador->n_objetos] = jugador->escenario->objetos[0];
                        jugador->n_objetos++;

                        printf("Has tomado el objeto %s (-1 TIEMPO)\n", jugador->escenario->objetos[0].nombre);
                        jugador->tiempo--;

                        jugador->escenario->objetos[0] = jugador->escenario->objetos[1];
                ...
                case '2':
                    if(jugador->escenario->cantidadObjetos > 1) {
                        jugador->inventario[jugador->n_objetos] = jugador->escenario->objetos[1];
                        jugador->n_objetos++;

                        printf("Has tomado el objeto %s (-1 TIEMPO)\n", jugador->escenario->objetos[1].nombre);
                        jugador->tiempo--;
                ...
                case 'w':
                    if(jugador->escenario->arriba) {
                        jugador->escenario = jugador->escenario->arriba;
                        flag = 1;
                    }
                    break;
                case 's':
                    if(jugador->escenario->abajo) {
                        jugador->escenario = jugador->escenario->abajo;
                        flag = 1;
                    }
                    break;
                case 'a':
                    if(jugador->escenario->izquierda) {
                        jugador->escenario = jugador->escenario->izquierda;
                        flag = 1;
                    }
                    break;
                case 'd':
                    if(jugador->escenario->derecha) {
                        jugador->escenario = jugador->escenario->derecha;
                        flag = 1;
                    }
                    break;
                }
            ...
            if(flag == 1)
            {
                jugador->tiempo -= ( (float) pesoTotal + 1) / 10;
            }
...
Partida *nuevaPartida() {
    ...
    if (!inicio) {
        printf("Error al cargar escenarios desde CSV\n");
        return NULL;
    }
    ...
    do {
        ...
        for(int i = 0; i < partida->n_jugadores; i++) {
            if(partida->jugadores[i].escenario->esFinal != 1) {
                flag = 0; // Si algun jugador no ha llegado al escenario final, continua.
            }
        }
